...
class uart_scoreboard extends cip_base_scoreboard #(
    .CFG_T(uart_env_cfg), .RAL_T(uart_reg_block), .COV_T(uart_env_cov)
);
    ...
    forever begin
      ...
      if (rx_enabled) begin
        ...
        if (parity_err) begin
          ...
          `uvm_info(
              `gfn, $sformatf("dropped uart rx item due to partiy err:\n%0s", item.sprint()),
                  UVM_HIGH
          )
        end
        if (!item.stop_bit) begin
          ...
          `uvm_info(
              `gfn, $sformatf("dropped uart rx item due to frame err:\n%0s", item.sprint()),
                  UVM_HIGH
          )
        end
        if (!parity_err && item.stop_bit == 1) begin  // no parity/frame error
          if (rx_q.size < UART_FIFO_DEPTH) begin
            ...
            `uvm_info(
                `gfn, $sformatf("dropped uart rx item due to overflow:\n%0s", item.sprint()),
                    UVM_HIGH
            )
          end
      ...
      "wdata": begin
        ...
        if (write && channel == AddrChannel) begin
          ...
          if (tx_q.size() < UART_FIFO_DEPTH) begin
            ...
            `uvm_info(
                `gfn, $sformatf("Drop tx item: %0h, tx_q size: %0d, uart_tx_clk_pulses: %0d",
                                csr.get_mirrored_value(), tx_q.size(), uart_tx_clk_pulses),
                    UVM_MEDIUM
            )
          end
          ...
          case (channel)
            ...
            DataChannel: begin  // check at data phase
              ...
              if (!(tx_enabled && tx_q_size_at_addr_phase == UART_FIFO_DEPTH &&
                ...
                `DV_CHECK_EQ(
                    get_field_val(ral.status.txfull, item.d_data), tx_full_exp, $sformatf(
                        "check tx_full fail: tx_en = %0d, tx_q.size = %0d, uart_tx_clk_pulses = %0d"
                            , tx_enabled, tx_q_size_at_addr_phase, uart_tx_clk_pulses)
                )
              end
              if (!(rx_enabled && is_in_ignored_period(UartRx) && rx_q_size_at_addr_phase inside {
                    UART_FIFO_DEPTH - 1, UART_FIFO_DEPTH})) begin
                `DV_CHECK_EQ(
                    get_field_val(ral.status.rxfull, item.d_data), rx_full_exp, $sformatf(
              ...
              if (!(tx_enabled && tx_q_size_at_addr_phase inside {0, 1} && is_in_ignored_period(
                    UartTx))) begin
                `DV_CHECK_EQ(
                    get_field_val(ral.status.txempty, item.d_data), tx_empty_exp, $sformatf(
                        "check tx_empty fail: uart_tx_clk_pulses = %0d, tx_q.size = %0d",
                            uart_tx_clk_pulses, tx_q_size_at_addr_phase)
                )
              end
              if (!(rx_q_size_at_addr_phase inside {0, 1} && is_in_ignored_period(UartRx))) begin
                `DV_CHECK_EQ(
                    get_field_val(ral.status.rxempty, item.d_data), rx_empty_exp, $sformatf(
                        "check rx_empty fail: uart_rx_clk_pulses = %0d, rx_q.size = %0d",
              ...
              if (!(tx_q_size_at_addr_phase == 0 && is_in_ignored_period(UartTx))) begin
                `DV_CHECK_EQ(
                    get_field_val(ral.status.txidle, item.d_data), tx_idle_exp, $sformatf(
              ...
              if (!(uart_rx_clk_pulses inside {1, uart_bits}) || !rx_enabled) begin
                `DV_CHECK_EQ(
                    get_field_val(ral.status.rxidle, item.d_data), rx_idle_exp, $sformatf(
                        "check rx_idle fail: rx_en = %0d, uart_rx_clk_pulses = %0d", rx_enabled,
                            uart_rx_clk_pulses)
                )
              end
            end
        ...
        if (write && channel == AddrChannel) begin  // write & address phase
          ...
          fork
            begin
              ...
              predict_rx_watermark_intr(
              .just_cleared(pre_intr[RxWatermark] & intr_wdata[RxWatermark]));
            end
          join_none
          ...
          foreach (intr_exp[i]) begin
            ...
            if (intr inside {TxWatermark, TxEmpty}) begin  // TX interrupts
              ...
              if (intr inside {RxTimeout, RxBreakErr} || is_in_ignored_period(UartRx)
                  ...
                  cfg.disable_scb_rx_frame_check && intr == RxFrameErr)) begin
                continue;
              end
            end
            `DV_CHECK_EQ(
                item.d_data[i], intr_exp_at_addr_phase[i], $sformatf("Interrupt: %0s", intr.name)
            );
            `DV_CHECK_CASE_EQ(
                cfg.intr_vif.pins[i], (intr_en[i] & intr_exp[i]), $sformatf(
          ...
          case (channel)
            ...
            DataChannel: begin  // check at data phase
              ...
              if (txlvl_act == txlvl_exp) begin
                // match, do nothing
              end else if (tx_enabled && is_in_ignored_period(UartTx) && absolute(
                           txlvl_act - txlvl_exp) == 1) begin
                // allow +/-1 difference as txlvl is reduced when previous item is in stop phase
                // match, do nothing
              end else begin
                `uvm_error(
                    `gfn, $sformatf(
                        "txlvl mismatch exp: %0d (+/-1), act: %0d,\
                                 clk_pulses: %0d"
                            , txlvl_exp, txlvl_act, uart_tx_clk_pulses)
                )
              end

              if (rxlvl_act == rxlvl_exp) begin
                // match, do nothing
              end else if (rx_enabled && is_in_ignored_period(UartRx) && absolute(
                           rxlvl_act - rxlvl_exp) == 1) begin
                // allow +/-1 difference as rxlvl is reduced when previous item is in stop phase
                // match, do nothing
              end else begin
                `uvm_error(
                    `gfn, $sformatf(
                        "rxlvl mismatch exp: %0d (+/-1), act: %0d,\
                                 clk_pulses: %0d"
                            , rxlvl_exp, rxlvl_act, uart_rx_clk_pulses)
                )
              end
              do_read_check = 1'b0;
  ...
  function void compare(uart_item act, uart_item exp, string dir = "TX");
    if (!act.compare(exp)) begin
      `uvm_error(
          `gfn, $sformatf("%s item mismatch!\nexp:\n%0s\nobs:\n%0s", dir, exp.sprint(), act.sprint()
              )
      )
    end else begin
      `uvm_info(
          `gfn, $sformatf("%s item match!\nexp:\n%0s\nobs:\n%0s", dir, exp.sprint(), act.sprint()),
              UVM_HIGH
      )
    end
  endfunction
