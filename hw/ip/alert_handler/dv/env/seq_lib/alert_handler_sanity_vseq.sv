...
class alert_handler_sanity_vseq extends alert_handler_base_vseq;
  ...
  task body();
    ...
    for (int i = 1; i <= num_trans; i++) begin
      ...
      `uvm_info(
          `gfn, $sformatf("starting seq %0d/%0d: intr_en=%0b, alert_trigger=%0b, alert_en=%0b", i,
                          num_trans, intr_en, alert_trigger, alert_en), UVM_LOW
      )
      ...
      if (ral.classa_ctrl.en_e0.get() | ral.classa_ctrl.en_e1.get() | ral.classa_ctrl.en_e2.get() |
          ral.classa_ctrl.en_e3.get()) begin
        clear_esc();
      end
      read_alert_cause();
    end
