...
class spi_device_scoreboard extends cip_base_scoreboard #(
    ...
    if (csr_addr inside {cfg.csr_addrs}) begin
      ...
      if (mem_addr inside {[tx_base : tx_base + tx_limit]}) begin  // TX address
        if (write && channel == AddrChannel) begin
          ...
          `uvm_info(
              `gfn, $sformatf("write tx_mem addr 0x%0h, data: 0x%0h", mem_addr - tx_base,
  ...
  virtual function void update_tx_fifo_and_rptr();
    ...
    while (tx_word_fifo.size < 2 && filled_bytes >= SRAM_WORD_SIZE) begin
      ...
      `uvm_info(
          `gfn, $sformatf("write tx_word_fifo rptr 0x%0h, data: 0x%0h", tx_rptr_exp, tx_mem.read(
                          tx_rptr_exp[SRAM_MSB:0])), UVM_MEDIUM
      )
  ...
  virtual function void receive_spi_rx_data(bit [TL_DW:0] data);
    if (get_rx_sram_space_bytes() >= SRAM_WORD_SIZE || rx_word_fifo.size < 2) begin
      ...
      `uvm_info(
          `gfn, $sformatf("RX overflow data: 0x%0h ptr: 0x%0h", data, rx_wptr_exp), UVM_MEDIUM
      )
    end
  endfunction
  ...
  virtual function void update_rx_mem_fifo_and_wptr();
    ...
    while (rx_word_fifo.size > 0 && space_bytes >= SRAM_WORD_SIZE) begin
      ...
      `uvm_info(
          `gfn, $sformatf("write rx_mem, addr 0x%0h, data: 0x%0h", rx_wptr_exp, data), UVM_MEDIUM
      )
  ...
  virtual function uint get_tx_sram_filled_bytes();
    ...
    uint filled_bytes = get_sram_filled_bytes(
        tx_wptr, tx_rptr_exp, `get_tx_allocated_sram_size_bytes, {`gfn, "::get_tx_sram_filled_bytes"
            });
    return filled_bytes;
  endfunction
  ...
  virtual function uint get_rx_sram_space_bytes();
    ...
    uint space_bytes = get_sram_space_bytes(
        rx_wptr_exp, rx_rptr, `get_rx_allocated_sram_size_bytes, {`gfn, "::get_rx_sram_space_bytes"}
        );
    return space_bytes;
  endfunction
