...
class spi_device_base_vseq extends cip_base_vseq #(
  ...
  virtual task spi_host_xfer_bytes(
      int num_bytes = $urandom_range(1, 512), ref logic [7:0] device_data[$]
  );
    spi_host_seq m_spi_host_seq = spi_host_seq::type_id::create("m_spi_host_seq");
    `uvm_create_on(m_spi_host_seq, p_sequencer.spi_sequencer_h)
  ...
  virtual task write_device_words_to_send(logic [31:0] device_data[$]);
    ...
    foreach (device_data[i]) begin
      ...
      `uvm_info(
          `gfn, $sformatf({"tx_wptr[SRAM_MSB:0] = 0x%0h, tx_wptr_phase_bit = 0x%0h, ",
                           "tx_sram_size_bytes = 0x%0h, tx_wptr_addr = 0x%0h"}, tx_wptr[SRAM_MSB:0],
                          tx_wptr[SRAM_PTR_PHASE_BIT], tx_sram_size_bytes, tx_wptr_addr), UVM_MEDIUM
      )
      tl_access(.addr(tx_wptr_addr), .write(1'b1), .data(device_data[i]));  // TODO: bkdr wr?

      // advance tx wptr by SRAM_WORD_SIZE
      tx_wptr = get_sram_new_ptr(
      .ptr(tx_wptr),
  ...
  virtual task read_host_words_rcvd(uint num_words, ref logic [31:0] host_data[$]);
    ...
    repeat (num_words) begin
      ...
      `uvm_info(
          `gfn, $sformatf({"rx_rptr[SRAM_MSB:0] = 0x%0h, rx_rptr_phase_bit = 0x%0h, ",
                           "rx_sram_size_bytes = 0x%0h, rx_rptr_addr = 0x%0h"}, rx_rptr[SRAM_MSB:0],
                          rx_rptr[SRAM_PTR_PHASE_BIT], rx_sram_size_bytes, rx_rptr_addr), UVM_MEDIUM
      )
  ...
  virtual task read_tx_avail_bytes(sram_avail_type_e avail_type, ref uint avail_bytes);
    ...
    endcase
    `uvm_info(
        `gfn, $sformatf("TX avail_type = %0s, avail_bytes = %0d", avail_type.name, avail_bytes),
            UVM_MEDIUM
    )
  endtask
  ...
  virtual task read_rx_avail_bytes(sram_avail_type_e avail_type, ref uint avail_bytes);
    ...
    endcase
    `uvm_info(
        `gfn, $sformatf("RX avail_type = %0s, avail_bytes = %0d", avail_type.name, avail_bytes),
            UVM_MEDIUM
    )
  endtask

  virtual task wait_for_tx_avail_bytes(
      uint req_bytes, sram_avail_type_e avail_type, ref uint avail_bytes
  );
    uint cur_bytes;
    `DV_SPINWAIT(
        do begin
        read_tx_avail_bytes(avail_type, cur_bytes);
            ...
            , {"wait_for_tx_avail_bytes::", avail_type.name}, default_timeout_ns * 2
    )
    avail_bytes = cur_bytes;
    `uvm_info(
        `gfn, $sformatf("TX req_bytes = %0d, avail_type = %0s, avail_bytes = %0d", req_bytes,
                        avail_type.name, avail_bytes), UVM_MEDIUM
    )
  endtask

  virtual task wait_for_rx_avail_bytes(
      uint req_bytes, sram_avail_type_e avail_type, ref uint avail_bytes
  );
    uint cur_bytes;
    `DV_SPINWAIT(
        ...
        read_rx_avail_bytes(avail_type, cur_bytes);
            ...
            , {"wait_for_rx_avail_bytes::", avail_type.name}, default_timeout_ns * 2
    )
    avail_bytes = cur_bytes;
    `uvm_info(
        `gfn, $sformatf("RX req_bytes = %0d, avail_type = %0s, avail_bytes = %0d", req_bytes,
                        avail_type.name, avail_bytes), UVM_MEDIUM
    )
  endtask
