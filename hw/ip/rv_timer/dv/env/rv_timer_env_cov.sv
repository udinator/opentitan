...
class rv_timer_cfg_cov_obj extends uvm_object;
  ...
  covergroup timer_cfg_cg(
      ...
      bit [7:0] step, bit [11:0] prescale, uint64 mtime, uint64 mtime_cmp
  );
...
class rv_timer_env_cov extends cip_base_env_cov #(.CFG_T(rv_timer_env_cfg));
  ...
  function new(string name, uvm_component parent);
    ...
    foreach (cfg_values_cov_obj[timer]) begin
      ...
      sticky_intr_cov[{"rv_timer_sticky_intr_pin", $sformatf("%0d", timer)}] = new(
      .name({"rv_timer_sticky_intr_pin", $sformatf("%0d", timer)}),
      .toggle_cov_en(0));
    end
    //Create toggle coverage for each prescale bit
    foreach (rv_timer_prescale_values_cov_obj[timer, bit_num]) begin
      rv_timer_prescale_values_cov_obj[timer][bit_num] = new(
          $sformatf("rv_timer-%0d-prescale-%0d", timer, bit_num));
    end
