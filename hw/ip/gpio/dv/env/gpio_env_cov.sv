...
class gpio_intr_type_cov_obj extends uvm_object;
  ...
  covergroup intr_type_cg(
      ...
      bit type_ctrl_en, bit intr_en, bit intr_state
  );
...
class gpio_env_cov extends cip_base_env_cov #(.CFG_T(gpio_env_cfg));
  ...
  // and data_in value
  covergroup data_out_data_oe_data_in_cross_cg(
      string name
  ) with function sample (
      uint pin, bit data_out, bit data_oe, bit data_in
  );
    option.name = name;
    ...
    cp_cross_all: cross cp_pin, data_out, data_oe, data_in{
      ...
      illegal_bins data_oe_1_data_out_0_data_in_1 = binsof (data_out) intersect {0} && binsof (
          data_oe) intersect {1} && binsof (data_in) intersect {1};
      illegal_bins data_oe_1_data_out_1_data_in_0 = binsof (data_out) intersect {1} && binsof (
          data_oe) intersect {1} && binsof (data_in) intersect {0};
    }
  endgroup : data_out_data_oe_data_in_cross_cg
  // Cross coverage between gpio pin value and effective data_in value
  covergroup gpio_pins_data_in_cross_cg(
      string name
  ) with function sample (
      uint pin, bit gpio_value, bit data_in
  );
    option.name = name;
    cp_pin: coverpoint pin {
      bins bins_for_gpio_bits[] = {[0 : NUM_GPIOS - 1]};
  ...
  function new(string name, uvm_component parent);
    ...
    begin
      ...
      string out_oe_reg_names[6] = '{"direct_out", "direct_oe", "masked_out_lower",
                                     "masked_oe_lower", "masked_out_upper", "masked_oe_upper"};
      foreach (intr_state_cov_obj[each_pin]) begin
        ...
        data_out_data_oe_cov_obj[each_pin] = new(
            $sformatf("data_out_data_oe_cov_obj_pin%0d", each_pin));
        ...
        sticky_intr_cov[{"gpio_sticky_intr_pin", $sformatf("%0d", each_pin)}] = new(
        .name({"gpio_sticky_intr_pin", $sformatf("%0d", each_pin)}),
        .toggle_cov_en(0));
      end
