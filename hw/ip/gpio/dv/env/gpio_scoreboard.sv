...
class gpio_scoreboard extends cip_base_scoreboard #(
      ...
      if (write) begin
        if (csr.get_name() == "intr_state") begin
          ...
          if (intr_state_update_queue.size() > 0) begin
            ...
            `uvm_info(
                `gfn, $sformatf("Entry taken out for clearing is %0p",
            ...
            for (uint each_bit = 0; each_bit < TL_DW; each_bit++) begin
              if (intr_state_write_to_clear_update.needs_update == 1'b1
                  && intr_state_write_to_clear_update.reg_value[each_bit] == 1'b1 && item.a_data[
                  each_bit] == 1'b1) begin
          ...
          if (csr.get_name() == "intr_test") begin
            ...
            if (cfg.en_cov && (!uvm_re_match("*out*", csr.get_name()) || !uvm_re_match(
                               "*oe*", csr.get_name()))) begin
  ...
  virtual task monitor_gpio_i();
    ...
    forever begin : monitor_pins_if
      ...
      `uvm_info(
          `gfn, $sformatf("cfg.gpio_vif.pins = %0h, under_reset = %0b", cfg.gpio_vif.pins,
                          under_reset), UVM_HIGH
      )
      if (under_reset == 1'b0) begin
        ...
        foreach (cfg.gpio_vif.pins_oe[pin_num]) begin
          ...
          `uvm_info(
              `gfn, $sformatf("pins_oe[%0d] = %0b pins_o[%0d] = %0b gpio_i_driven[%0d] = %0b",
                              pin_num, cfg.gpio_vif.pins_oe[pin_num], pin_num, cfg.gpio_vif.pins_o[
                              pin_num], pin_num, gpio_i_driven[pin_num]), UVM_HIGH
          )
        end

        `uvm_info(
            `gfn, $sformatf("pins = 0x%0h [%0b]) gpio_i_driven = 0x%0h [%0b]", cfg.gpio_vif.pins,
                            cfg.gpio_vif.pins, gpio_i_driven, gpio_i_driven), UVM_HIGH
        )
        ...
        if (prev_gpio_i !== cfg.gpio_vif.pins) begin
          ...
          foreach (gpio_i_transition[ii]) begin
            `uvm_info(
                `gfn, $sformatf("gpio_i_transition[%0d] = %0p", ii, gpio_i_transition[ii]), UVM_HIGH
            )
          end
          ...
          `uvm_info(
              `gfn, $sformatf("updated prev_gpio_i = 0x%0h [%0b]", prev_gpio_i, prev_gpio_i),
                  UVM_HIGH
          )
        end
        ...
        `uvm_info(
            `gfn, $sformatf("prv_gpio_i_pins_o = 0x%0h [%0b]", prv_gpio_i_pins_o, prv_gpio_i_pins_o
                ), UVM_HIGH
        )
        `uvm_info(
            `gfn, $sformatf("prv_gpio_i_pins_oe = 0x%0h [%0b]", prv_gpio_i_pins_oe,
                            prv_gpio_i_pins_oe), UVM_HIGH
        )
      end
  ...
  function void gpio_predict_and_compare(uvm_reg csr = null);
    ...
    if (csr != null) begin
      ...
      case (csr.get_name())
        ...
        "direct_out": begin
          ...
          `uvm_info(
              `gfn, $sformatf("data_out updated to 0x%0h [%0b]", data_out, data_out), UVM_HIGH
          )
        ...
        "masked_oe_lower": begin
          ...
          `uvm_info(
              `gfn, $sformatf("data_oe reg updated to 0x%0h [%0b]", data_oe, data_oe), UVM_HIGH
          )
          // Update mirror values of *oe* registers
          update_gpio_oe_regs();
        end
    ...
    if (gpio_data_check == 1'b1) begin
      ...
      `uvm_info(
          msg_id, $sformatf("data_out_effect_on_gpio_i = 0x%0h [%0b]", data_out_effect_on_gpio_i,
                            data_out_effect_on_gpio_i), UVM_HIGH
      )
      `uvm_info(
          msg_id, $sformatf("gpio_i_driven = 0x%0h [%0b]", gpio_i_driven, gpio_i_driven), UVM_HIGH
      )
      ...
      `uvm_info(
          msg_id, $sformatf("pred_val_gpio_pins = %0h(%0b)", pred_val_gpio_pins, pred_val_gpio_pins
              ), UVM_HIGH
      )
  ...
  virtual function void gpio_interrupt_predict(
      ...
      if (cfg.en_cov) begin
        foreach (rising_edge_intr_events[each_bit]) begin
          cov.intr_event_type_cov_objs[each_bit]["intr_event_rising"].intr_type_cg.sample(
              intr_ctrl_en_rising[each_bit], intr_enable[each_bit], rising_edge_intr_events[each_bit
                  ]);
          cov.intr_event_type_cov_objs[each_bit]["intr_event_falling"].intr_type_cg.sample(
              intr_ctrl_en_falling[each_bit], intr_enable[each_bit], falling_edge_intr_events[
                  each_bit]);
        end
      end
    end
    ...
    `uvm_info(
        msg_id, $sformatf("Predicted interrupt status = 0x%0h [%0b]", exp_intr_status,
                          exp_intr_status), UVM_HIGH
    )
    begin
  ...
  function void sample_data_out_data_oe_coverage();
    if (cfg.en_cov) begin
      for (uint each_bit = 0; each_bit < NUM_GPIOS; each_bit++) begin
        cov.data_out_data_oe_cov_obj[each_bit].var1_var2_cg.sample(
            data_out[each_bit], data_oe[each_bit]);
      end
    end
