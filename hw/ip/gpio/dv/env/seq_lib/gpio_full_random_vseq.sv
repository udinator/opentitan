...
class gpio_full_random_vseq extends gpio_random_long_reg_writes_reg_reads_vseq;
  ...
  task body();
    ...
    for (uint tr_num = 0; tr_num < num_trans; tr_num++) begin
      ...
      randcase
        ...
        1: begin
          `uvm_info(
              msg_id, $sformatf("Latest data_out = 0x%0h [%0b]", data_out, data_out), UVM_HIGH
          )
  ...
  task pgm_out_oe_regs(
      bit [NUM_GPIOS-1:0] gpio_if_pins_o_val, bit [NUM_GPIOS-1:0] gpio_if_pins_oe_val
  );
    ...
    if ($urandom_range(0, 1)) begin
      ...
      for (uint pin = 0; pin < ral.masked_out_upper.mask.get_n_bits(); pin++) begin
        if ((data_oe[(NUM_GPIOS / 2) + pin] == 1'b1) && (gpio_if_pins_oe_val[(NUM_GPIOS / 2) + pin]
                                                         == 1'b1) && (
    ...
    if ($urandom_range(0, 1)) begin
      ...
      for (uint pin = 0; pin < ral.masked_oe_upper.mask.get_n_bits(); pin++) begin
        if (mask[pin] == 1'b1 && data[pin] == 1'b1 && gpio_if_pins_oe_val[(TL_DW / 2) + pin] == 1'b1
