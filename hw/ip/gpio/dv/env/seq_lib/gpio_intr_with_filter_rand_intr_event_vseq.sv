...
class gpio_intr_with_filter_rand_intr_event_vseq extends gpio_base_vseq;
    ...
    forever
      @(cfg.clk_rst_vif.cb) begin
        ...
        foreach (crnt_pins_val[each_pin]) begin
          ...
          pins_stable_period_and_filter_cg.sample(
              each_pin, stable_cycles_cnt[each_pin], crnt_pins_val[each_pin], gpio_filter_value[
                  each_pin]);
        end
  ...
  task body();
    ...
    for (uint tr_num = 0; tr_num < num_trans; tr_num++) begin
      ...
      `uvm_info(
          msg_id, $sformatf("crnt_intr_status = 0x%0h [%0b]", crnt_intr_status, crnt_intr_status),
              UVM_HIGH
      )
      ...
      `uvm_info(
          msg_id, $sformatf("gpio_filter_value = 0x%0h [%0b]", gpio_filter_value, gpio_filter_value
              ), UVM_HIGH
      )
      ...
      fork
        ...
        begin : csr_read_and_check
          ...
          bit [NUM_GPIOS-1:0] expected_value_data_in = predict_data_in_value(
              gpio_filter_value, stable_value, gpio_i);
          // Predict updated interrupt status register again
          ...
          `uvm_info(
              `gfn, $sformatf("Expected data in = 0x%0h [%0b]", expected_value_data_in,
                              expected_value_data_in), UVM_HIGH
          )
          fork
            begin : count_clock
      ...
      join begin : wait_all_pins_stable
        ...
        repeat (additional_clk_cycles_until_all_gpio_pins_stable) begin
          ...
          foreach (stable_cycles_per_pin[pin]) begin
            ...
            if (cycles_after_16_per_pin[pin] == stable_cycles_per_pin[pin]) begin
              latest_stable_value[pin] = ~gpio_i[pin];
              `uvm_info(
                  msg_id, $sformatf("updating latest_stable_value[%0d] to %0b", pin,
                                    latest_stable_value[pin]), UVM_HIGH
              )
            end
          end
  ...
  function bit [NUM_GPIOS-1:0] predict_data_in_value(
    ...
    for (uint i = 0; i < NUM_GPIOS; i++) begin
      ...
      `uvm_info(
          msg_id, {$sformatf("pin-%0d filter_en[%0d] = %0b", i, i, filter_en[i]), $sformatf(
                   "crnt_filtered_value[%0d] = %0b", i, crnt_filtered_value[i]), $sformatf(
                   "gpio_i[%0d] = %0b", i, gpio_i[i])}, UVM_HIGH
      )
    end
  ...
  function void update_intr_state(
      ref bit [TL_DW-1:0] current_interrupt_state,
