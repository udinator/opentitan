...
class gpio_filter_stress_vseq extends gpio_intr_with_filter_rand_intr_event_vseq;
            ...
            fork
              begin
                if (gpio_filter_value[pin] == 1'b1) begin
                  ...
                  update_pin_intr_state(pin, intr_state_this_pin, stable_value[pin], ~stable_value[
                                        pin]);
    ...
    fork
      begin : keep_driving_noise
        while (async_drive_time_accumulated < (noise_length_cycles * clk_period_ps)) begin
          uint ps_delay;
          `DV_CHECK_STD_RANDOMIZE_WITH_FATAL(
              ps_delay, ps_delay inside {[1:((noise_length_cycles * clk_period_ps) - 1)]};
          )
          ...
          if ((async_drive_time_accumulated + ps_delay) > (noise_length_cycles * clk_period_ps)
              ) begin
  ...
  function void update_pin_intr_state(
      input uint pin_idx,
  ...
  task read_and_check(
      bit [NUM_GPIOS-1:0] predicted_data_in, bit [NUM_GPIOS-1:0] predicted_intr_state
  );
