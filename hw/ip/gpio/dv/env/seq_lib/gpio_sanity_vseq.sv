...
class gpio_sanity_vseq extends gpio_base_vseq;
  ...
  task body();
    ...
    for (uint tr_num = 0; tr_num < num_trans; tr_num++) begin
      ...
      `uvm_info(
          msg_id, {$sformatf("reading data_in after %0d clock cycles ", delay), $sformatf(
                   "csr_rd_val = %0h", csr_rd_val)}, UVM_HIGH
      )
    end
    ...
    for (uint tr_num = 0; tr_num < num_trans; tr_num++) begin
      ...
      `uvm_info(
          msg_id, $sformatf("writing direct_out = 0x%0h [%0b] direct_oe = 0x%0h [%0b]", gpio_o,
                            gpio_o, gpio_oe, gpio_oe), UVM_HIGH
      )
