  ...
  virtual task process_tl_access(tl_seq_item item, tl_channels_e channel = DataChannel);
        ...
        case (csr_name)
          "cmd": begin
            if (sha_en) begin
              ...
              void'(ral.intr_state.hmac_err.predict(.value(1), .kind(UVM_PREDICT_DIRECT)));
              void'(ral.err_code.predict(
              .value(SwHashStartWhenShaDisabled),
          ...
          "key0", "key1", "key2", "key3", "key4", "key5", "key6", "key7": begin
            ...
            if (hmac_start) begin
              void'(ral.intr_state.hmac_err.predict(.value(1), .kind(UVM_PREDICT_DIRECT)));
              void'(ral.err_code.predict(
              .value(SwUpdateSecretKeyInProcess),
              .kind(UVM_PREDICT_DIRECT)));
              return;
            end
          ...
          "digest0", "digest1", "digest2", "digest3", "digest4", "digest5", "digest6", "digest7",
            ...
            `uvm_error(
                `gfn, $sformatf("this reg does not have write access: %0s", csr.get_full_name())
            )
          end
          default: begin
    ...
    if (!write) begin
      ...
      if (do_read_check) begin
        `uvm_info(
            `gfn, $sformatf("%s reg is checked with expected value %0h", csr_name,
                            csr.get_mirrored_value()), UVM_HIGH
        );
          ...
          fork
            begin : increase_wr_cnt
              ...
              if (sha_en) begin
                ...
                if (fifo_full) begin
                  wait(hmac_wr_cnt - hmac_rd_cnt < HMAC_MSG_FIFO_DEPTH) fifo_full = 0;
                end
