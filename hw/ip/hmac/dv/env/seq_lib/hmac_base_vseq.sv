  ...
  virtual task wr_msg(bit [7:0] msg[]);
    ...
    while (msg_q.size() > 0) begin
      ...
      if (msg_q.size() < 4) begin
        `DV_CHECK_FATAL(
            randomize(wr_size, wr_addr, wr_mask) with {
              1 << wr_size <= msg_q.size();
            }
        )
      end else begin
      ...
      `uvm_info(
          `gfn, $sformatf("wr_size = %0h, wr_addr = %0h, wr_mask = %0h, words = 0x%0h", wr_size,
