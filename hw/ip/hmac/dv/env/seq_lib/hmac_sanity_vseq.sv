...
class hmac_sanity_vseq extends hmac_base_vseq;
  ...
  task body();
    ...
    for (int i = 1; i <= num_trans; i++) begin
      ...
      `uvm_info(
          `gfn, $sformatf("starting seq %0d/%0d, message size %0d, hmac=%0d, sha=%0d", i, num_trans,
                          msg.size(), hmac_en, sha_en), UVM_LOW
      )
      `uvm_info(
          `gfn, $sformatf("intr_fifo_full/hmac_done/hmac_err_en=%b, endian/digest_swap=%b", {
      ...
      .intr_fifo_full_en(intr_fifo_full_en),
