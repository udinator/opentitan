...
class aes_scoreboard extends cip_base_scoreboard #(
  ...
  mailbox #(aes_seq_item) ref_fifo
      ;  // this will be a clone of the above before the result has been calculated by the dut!
  ...
  virtual task compare();
    ...
    forever begin
      ...
      `uvm_info(
          `gfn, $sformatf("\n\t ----| printing C MODEL %s", c_item.convert2string()), UVM_HIGH
      )
